...
- `DB_BACKEND=mysql`
- `DB_PATH`（可选，SQLite 文件路径；默认 `./quiz.db`，Vercel 下为只读包内文件）
...
> - `AI_SETTINGS_PATH`（可选；默认 `./ai_settings.json`，Vercel 下为只读包内文件）
...
- Vercel 部署时，项目文件系统是只读（`/tmp` 例外）。本项目默认读取包内文件：
  - SQLite 路径：`./quiz.db`
  - AI 设置路径：`./ai_settings.json`
- 若需要在线写入与持久化，建议使用 MySQL；不要依赖实例本地文件。
